...
function Viewer() {
  ...
  const [viewer, setViewer] = useState({
    username: "Anonyme-" + Math.random().toString(36).substring(2),
  });
  ...
  useEffect(() => {
    API.get(`/users/live/${username}`)
      .then((response) => {
        ...
        if (user) setViewer({ username: user.username });
  ...
  useEffect(() => {
    ...
    socketRef.current.on("users:exit", ({ users, room }) => {
      setUsers(users);
  ...
  return (
    <div>
      <div className="mt-6">
        <div className="2xl:flex-row flex-col flex gap-6">
            ...
            <div className="flex gap-5">
              <div className="self-center 2xl:self-start relative rounded-xl overflow-hidden aspect-video w-full h-fit">
                ...
                <div className="absolute top-3 left-4 right-4 flex gap-2 items-center justify-between">
                  <h3 className="block text-lg font-semibold px-4 py-2 rounded-lg bg-slate-800/50 backdrop-blur-xl text-slate-300 border-slate-600 focus:ring-blue-300 focus:ring-opacity-40 focus:border-blue-300 focus:outline-none focus:ring">
                    {live.title}
                  </h3>
                  <h3 className="block  font-semibold text-slate-300 border-slate-600 focus:ring-blue-300 focus:ring-opacity-40 focus:border-blue-300 focus:outline-none focus:ring">
                    @{live.username}
                  </h3>
                </div>
          ...
          <div className="lg:min-w-[400px] bg-slate-800 rounded-xl flex flex-col">
            ...
            <div
              ref={chat}
              className="flex flex-col basis-full min-h-[300px] max-h-[600px] overflow-y-scroll gap-4 p-3"
            >
